...
export default function PositionProgressChart({ history }) {
  if (!Array.isArray(history) || history.length === 0) {
    return <Text color="gray.400">Sin historial de posiciones.</Text>;
  }

  // history: [{ jornada, position }]
  return (
    <Box w="100%" h="220px" bg="white" borderRadius="md" boxShadow="sm" p={4}>
      <Text fontWeight="bold" mb={2} fontSize="md">Progreso de posición por jornada</Text>
      <ResponsiveContainer width="100%" height={160}>
        <LineChart data={history} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
          ...
          <Line type="monotone" dataKey="position" stroke="#3182ce" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
        </LineChart>
      </ResponsiveContainer>
    </Box>
  );
