...
// Devuelve un array: [{ jornada, points, rank, totalPoints }]
export default function useCumulativePointsHistory(participantId) {
  ...
  useEffect(() => {
    ...
    fetch("/api/participants/leaderboard")
      ...
      .then((data) => {
        ...
        const participantHistory = allJornadas.map((j, idx) => {
          ...
          return {
            jornada: j,
            totalPoints: cumulativeByParticipant[participantId]?.[idx]?.totalPoints || 0,
